...
from consistency_arm_policy import ConsistencyArmActionCfg
...
class Go2ExteroceptionObsCfg(ObsGroup):
    ...
    # Innovation 3: replace raw lidar scan with c_v from StalenessAwareVisualEncoder
    visual_context = ObsTerm(
        func=lambda env: env.c_v,
    )
...
class ArmedGo2ActionsCfg:
    ...
    arm: ConsistencyArmActionCfg = ConsistencyArmActionCfg(
        asset_name="robot",
