"""Below are mostly copied"""
## 🛠 Environment Alignment (Critical)

> **WARNING**: This project requires NVIDIA Driver **570.169+**. 
> [cite_start]Running on 560.94 seems would trigger Vulkan `ERROR_INCOMPATIBLE_DRIVER` and fail to initialize GPU. [cite: 10, 77]
(However,I remember I failed at newer Driver and I tried to do it at --headless. Find time to check out)

| Component           | Requirement / Version                                     |
|:--------------------|:----------------------------------------------------------|
| **Isaac Sim** | [cite_start]`5.1.0` (Internal Kit 107.3.3) [cite: 4, 6]               |
| **Isaac Lab** | `0.54.3`                                                  |
| **Base OS** | [cite_start]`Ubuntu 24.04.2 LTS` (Noble Numbat) [cite: 74]            |
| **Python** | [cite_start]`3.11` (via Isaac Sim bundled python) [cite: 2]           |
| **Container** | [cite_start]Docker (Requires NVIDIA Container Toolkit) [cite: 74]     |
| **GPU** | [cite_start]NVIDIA RTX 4090 (Verified) [cite: 11]                     |



[INFO]: Time taken for simulation start : 8.982185 seconds
Traceback (most recent call last):
  File "/workspace/isaaclab/isaaclab_master/train.py", line 1028, in <module>
    main()
  File "/workspace/isaaclab/isaaclab_master/train.py", line 978, in main
    env = gym.make(
          ^^^^^^^^^
  File "/workspace/isaaclab/_isaac_sim/kit/python/lib/python3.11/site-packages/gymnasium/envs/registration.py", line 734, in make
    env = env_creator(**env_spec_kwargs)
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/workspace/isaaclab/isaaclab_master/env_manager.py", line 52, in __init__
    super().__init__(cfg, **kwargs)
  File "/workspace/isaaclab/source/isaaclab/isaaclab/envs/manager_based_rl_env.py", line 80, in __init__
    super().__init__(cfg=cfg)
  File "/workspace/isaaclab/source/isaaclab/isaaclab/envs/manager_based_env.py", line 173, in __init__
    self.sim.reset()
  File "/workspace/isaaclab/source/isaaclab/isaaclab/sim/simulation_context.py", line 530, in reset
    self.render()
  File "/workspace/isaaclab/source/isaaclab/isaaclab/sim/simulation_context.py", line 601, in render
    raise exception_to_raise
  File "/workspace/isaaclab/source/isaaclab/isaaclab/assets/asset_base.py", line 317, in _initialize_callback
    self._initialize_impl()
  File "/workspace/isaaclab/source/isaaclab/isaaclab/assets/articulation/articulation.py", line 1583, in _initialize_impl
    self._process_cfg()
  File "/workspace/isaaclab/source/isaaclab/isaaclab/assets/articulation/articulation.py", line 1684, in _process_cfg
    indices_list, _, values_list = string_utils.resolve_matching_names_values(
                                   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/workspace/isaaclab/source/isaaclab/isaaclab/utils/string.py", line 369, in resolve_matching_names_values
    raise ValueError(
ValueError: Not all regular expressions are matched! Please check that the regular expressions are correct: 
    .*_hip_joint: ['FL_hip_joint', 'FR_hip_joint', 'RL_hip_joint', 'RR_hip_joint']
    .*_thigh_joint: ['FL_thigh_joint', 'FR_thigh_joint', 'RL_thigh_joint', 'RR_thigh_joint']
    .*_calf_joint: ['FL_calf_joint', 'FR_calf_joint', 'RL_calf_joint', 'RR_calf_joint']
    joint[1-6]: []
...
Error Analysis
The error joint[1-6]: [] indicates that the ArticulationView index in Isaac Lab completely failed to resolve any joints matching joint1 through joint6.

Critical Insight
The Available strings list provided in the traceback only contains the 12 leg joints of the Go2 base. This confirms that the entire subtree corresponding to the Z1 arm was "pruned" (culled) during the physics engine's loading phase.

PhysX Mechanism
In PhysX, if a subtree within an Articulation contains any Rigid Body marked as Kinematic rather than Dynamic, or if a FixedJoint connection is improperly configured, the engine will silently cull the entire non-compliant subtree to guarantee solver stability.
